package com.dabom.messaging.kafka.event;
...
public class KafkaEventMessageSupport {
    ...
    public <T> void consumeByEventType(
        ...
        try {
            // 먼저 eventType만 확인해서 관심 없는 이벤트는 역직렬화 전에 걸러낸다.
            ...
            if (!expectedEventType.equals(actualEventType)) {
                log.warn(
                        "Skip unexpected event. topic={}, recordKey={}, expectedEventType={}, actualEventType={}",
            ...
            log.error(
                    ...
                    logSanitizer.sanitize(consumerRecord.key()),
                    e);
            throw new KafkaMessageDeserializationException("Failed to parse kafka payload", e);
        } catch (Exception e) {
            log.error(
